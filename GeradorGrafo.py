...
class GeradorGrafo:
    ...
    def __init__(self, board):
        ...
        self.playerAtual()
        self.root = self.setaNodo(Node(board), PROFUNDIDADE+1)
    ...
    def setaNodo(self, nodo, profundidade):
        ...
        nodo.stepsPlayer = Objetivo().steps_to_win(nodo.board, self.__player) * 2
        nodo.stepsPlayer -= (1 if nodo.proxPlayer == self.__player else 0)
        self.bestSolution = min(self.bestSolution, nodo.stepsPlayer)
        nodo.stepsOpponent = Objetivo().steps_to_win( nodo.board, self.__adversario) * 2
        nodo.stepsOpponent -= (1 if nodo.proxPlayer == self.__adversario else 0)
